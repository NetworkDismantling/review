...
#################################################New code for FINDER#####################################
    ...
    def EvaluateRealData(self,
                         # model_file, # MARCO: This parameter was not used
                         # MARCO: Instead of passing the file, we pass the network object already
                         g,
                         # data_test,
                         # save_dir,
                         # result_file,
        ...
        # MARCO: Do not compute the output directory here, just use the one provided.
        ...
        # MARCO: No need to write on file. We return the solution
        ...
        # print("Inserting graph", flush=True)
        self.InsertGraph(g, is_test=True)
        # print("Done inserting graph", flush=True)
        t1 = time.time()

        # print("Getting the solution")
        solution = self.GetSolution(0, step)

        # print("Done getting the solution")
        t2 = time.time()
        solution_time = (t2 - t1)
    ...
    def EvaluateSol(self, g,
        ...
        sys.stdout.flush()
        # MARCO: No need to read the network from file again. We just get it from the parameters
        # g = nx.read_edgelist(data_test)
        ...
        # MARCO: No need to load the solution back from file. We just get it from the parameters
        # sol = []
    ...
    def GenNetwork(self, g):    #networkx2four
        ...
        # print("Returning generated network...", flush=True)
